...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    if args.model_type in ['mcsp_otmr', 'mcat']:
        from models.model_coattn import MCSP_OTMR
        g_model_type = args.g_model_type if args.generator else None 
        model_dict = {'use_condition': args.g_condition, 'g_model_type': g_model_type, 'generator':args.generator, 'wsi_encoding_dim': args.wsi_encoding_dim, \
                      'fusion': args.fusion, 'omic_sizes': args.omic_sizes, 'n_classes': args.n_classes, \
                      'alpha_surv': args.alpha_surv, 'decoder_mode': args.decoder_mode}
        model = MCSP_OTMR(**model_dict)
    ...
    ##########fl
    num_class = [0,0,0,0]
    for _, _, _, _, _, _, _, label, _, _, _ in train_loader:
        num_class[label] += 1
    ...
    args.annealing_agent = Annealer(args.max_epochs*len(train_loader)/args.gc, shape='cosine')  # instantiating annealing agent
    ########fisher
    all_fisher_path = []
    all_fisher_omic = []
    
    wsi_omic_fisher_list = {'slide_id':[], 'wsi_fisher':[], 'omic_fisher':[], 'wsi_feature':[], 'omic_feature':[],'label':[], 'event_time':[]}
    fisher = {"wsi": 0.0, "omic": 0.0}
    # ########
    all_epoch_sample_wsi_fisher = [] 
    all_epoch_sample_omic_fisher = [] 
    ...
    for epoch in range(args.start_epoch,args.max_epochs):
        if args.mode == 'coattn':
            if args.model_type in ['mcsp_otmr', 'mcat']:
                from trainer.coattn_trainer import train_loop_survival_coattn, validate_survival_coattn, validate_survival_coattn_missing,validate_survival_coattn_patch_zero,validate_survival_coattn_heatmap
                fisher, wsi_omic_fisher_list = train_loop_survival_coattn(epoch, model, train_loader, optimizer, args.n_classes, writer, loss_fn, reg_fn, args.lambda_reg, args.gc, args, wsi_omic_fisher_list, fisher) #fffff
                print('fish:',fisher,epoch) #fffff
                ...
                if args.generator:
                    val_missing_latest, c_index_missing, _ = validate_survival_coattn_missing(cur, epoch, model, val_loader, args.n_classes, early_stopping, monitor_cindex, writer, loss_fn, reg_fn, args.lambda_reg, args.results_dir, args)
        ...
        if c_index_val > max_c_index:
            ...
            best_val_dict = val_latest#val_missing_latest #val_latest
