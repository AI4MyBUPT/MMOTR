...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self, csv_path = 'dataset_csv/ccrcc_clean.csv', mode = 'omic', apply_sig = False, 
        ...
        missing_slides_csv = os.path.join(PROJECT_ROOT, 'dataset_csv', 'missing_h5.csv')
        ...
        if self.apply_sig:
            self.signatures = pd.read_csv(os.path.join(PROJECT_ROOT, 'datasets_csv_sig', 'signatures.csv'))
        else:
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == 'path':
                    ...
                    return (patch_features, omic1, omic2, omic3, omic4, omic5, omic6, label, event_time, c, slide_id)
