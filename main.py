...
parser.add_argument('--split_dir',       type=str, default=f'tcga_brca',
...
### Model Parameters.
parser.add_argument('--model_type',      type=str, choices=['mcsp_otmr', 'mcat'], 
                    default='mcsp_otmr', help='Type of model (Default: mcsp_otmr)')
...
parser.add_argument('--wsi_encoding_dim',type=int, default=768, help='wsi encoding dim.')
parser.add_argument('--fusion',          type=str, choices=['None', 'concat'], default='concat', help='Type of fusion. (Default: concat).')
parser.add_argument('--generator',		 action='store_true', default=True, help='Use OTCR to reconstruct missing genomic modality.')
parser.add_argument('--g_model_type',		 type=str,  default='OT', help='Type of missing-modality reconstructor. Default: OT for OTCR.')
parser.add_argument('--g_condition',	action='store_true', default=True, help='Use pathology-conditioned genomic reconstruction.') 
parser.add_argument('--decoder_mode',	 type=str,  default='specific', help='Decoder mode for OTCR reconstruction. (specific, shared)')
...
parser.add_argument('--gc',              type=int,
                    default=1, help='Gradient Accumulation Step.')
...
parser.add_argument('--alpha',           type=float, default=0.1,
                    help='alpha value for alignment loss')
parser.add_argument('--beta',           type=float, default=1.,
                    help='beta value for vib-trans loss')

parser.add_argument('--missing_rate',    type=float, default=1.0,
                    help='missing rate for genomic data')
...
for data_index in [0]:
    args.split_dir = dataset_name[data_index]
    args.data_root_dir = data_pathology[data_index]
    args.feature_extractor = 'SwinTrans' 
    args.wsi_encoding_dim = 768 
    args.fusion = 'concat' 
    args.mode = 'coattn' 
    ...
    if 'survival' in args.task:
        ...
        if combined_study in ['tcga_blca', 'tcga_brca','tcga_gbmlgg', 'tcga_ucec', 'tcga_luad']:
            csv_path = os.path.join(PROJECT_ROOT, args.dataset_path, '%s_all_clean.csv.zip' % combined_study)
        else:
            csv_path = os.path.join(PROJECT_ROOT, args.dataset_path, '%s_all_clean.csv' % combined_study)
    ...
    args.results_dir = os.path.join(args.results_dir, args.split_dir, args.which_splits, args.feature_extractor, args.param_code, exp_code)
    if not os.path.isdir(args.results_dir):
    ...
    args.split_dir = os.path.join(PROJECT_ROOT, 'splits', args.which_splits, args.split_dir)
